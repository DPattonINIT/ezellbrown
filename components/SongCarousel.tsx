...
const SongCarousel = ({ songs, onSelectSong, currentSongIndex }: Props) => {
  ...
  return (
    <div
  ref={containerRef}
  className="overflow-x-auto whitespace-nowrap py-4 px-2 mt-4 flex items-center scroll-smooth show-scrollbar"
>
      ...
      {songs.map((song, index) => (
        ...
        >
          <div
            className={`relative w-24 h-24 rounded-lg overflow-hidden cursor-pointer border-4 transition-transform hover:scale-105 hover:opacity-90 ${
              index === currentSongIndex
                ? "border-ezellYellow ring-2 ring-ezellYellow shadow-glow"
                : "border-white"
            }`}
            onClick={() => onSelectSong(index)}
            title={`Play ${song.title}`}
          ...
          <p className="text-[10px] text-white mt-2 font-semibold leading-tight truncate" title={song.title}>
            {song.title.split(" - ")[1]?.split("(")[0] || song.title}
          </p>
        </div>
      ))}
