...
// =======================================Updated admin with image upload for info, download toggle============
'use client';
import { useEffect, useState, useCallback } from 'react';
import { supabase } from '@/lib/supabase';
import toast, { Toaster } from 'react-hot-toast';
...
const AdminPage = () => {
  ...
  const [infoImages, setInfoImages] = useState<InfoImage[]>([]);
  const [editingEvent, setEditingEvent] = useState<Event | null>(null);
  const [view, setView] = useState<'create' | 'manage' | 'songs' | 'info-images' | 'analytics'>('create');
  ...
  const fetchInfoImages = useCallback(async () => {
    if (!session?.user?.id) return;
    ...
    if (!error) {
      ...
      toast.error('Failed to fetch info images');
    }
  }, [session]);
  ...
  }, [session, fetchEvents, fetchSongs, fetchInfoImages]);
  ...
  const toggleSongDownload = async (song: Song) => {
    toast.loading('Updating song...');
    try {
      const { error } = await supabase
        .from('songs')
        .update({ downloadable: !song.downloadable })
        .eq('id', song.id)
        .eq('user_id', session?.user?.id);
      ...
      toast.dismiss();
      toast.success(`Download ${!song.downloadable ? 'enabled' : 'disabled'}!`);
      fetchSongs();
    } catch (err: unknown) {
      toast.dismiss();
      if (err instanceof Error) {
  ...
  const handleUploadInfoImage = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget;
    const formData = new FormData(form);
    const file = formData.get('infoImage') as File;
    ...
    try {
      ...
      const maxOrder = infoImages.length > 0 
        ? Math.max(...infoImages.map(img => img.order_index))
        : -1;
  ...
  const handleDeleteInfoImage = async (image: InfoImage) => {
    ...
    try {
      ...
      await supabase.storage.from('info-images').remove([fileName!]);
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="relative mb-8">
          <h1 className="text-3xl font-bold text-center underline mb-6">ADMIN DASHBOARD</h1>
          <div className="flex items-center flex-wrap gap-4">
            <div className="flex gap-4 flex-wrap">
              <button onClick={() => setView('create')} className={`px-4 py-2 rounded cursor-pointer ${view === 'create' ? 'adminYellow' : 'bg-gray-700 text-white'}`}>
                {editingEvent ? 'Edit Event' : 'Create Event'}
              ...
              <button onClick={() => setView('songs')} className={`px-4 py-2 rounded cursor-pointer ${view === 'songs' ? 'adminYellow' : 'bg-gray-700 text-white'}`}>
                Song Downloads ({songs.length})
              </button>
              <button onClick={() => setView('info-images')} className={`px-4 py-2 rounded cursor-pointer ${view === 'info-images' ? 'adminYellow' : 'bg-gray-700 text-white'}`}>
                Info Page Images ({infoImages.length})
              </button>
              <button onClick={() => setView('analytics')} className={`px-4 py-2 rounded cursor-pointer ${view === 'analytics' ? 'adminYellow' : 'bg-gray-700 text-white'}`}>
                Analytics
              </button>
            </div>
            <div className="bg-gray-800 p-2 px-4 rounded text-sm border border-gray-700 ml-auto">
        ...
        {view === 'songs' && (
          <div>
            <h2 className="text-2xl font-bold mb-4">SONG DOWNLOAD CONTROLS</h2>
            {songs.length === 0 ? (
              <p className="text-gray-400">No songs found in database.</p>
            ) : (
              <div className="space-y-4">
                {songs.map((song) => (
                  <div key={song.id} className="bg-[#1a1a1a] p-4 rounded-lg flex items-center justify-between border border-gray-700">
                    <div>
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm font-medium text-white">
                        {song.downloadable ? '✓ Downloadable' : '✗ Stream Only'}
                      </span>
                      <button
                        onClick={() => toggleSongDownload(song)}
                        className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded font-semibold cursor-pointer transition-colors border border-gray-600"
                      >
                        Toggle Download
                      </button>
                    </div>
        ...
        {view === 'info-images' && (
          <div>
            <h2 className="text-2xl font-bold mb-4">INFO PAGE IMAGES</h2>
            
            <div className="bg-[#1a1a1a] p-6 rounded-lg mb-8 border border-gray-700">
              <h3 className="text-xl font-semibold mb-4">Upload New Image</h3>
              <form onSubmit={handleUploadInfoImage} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    accept="image/*" 
                    required
                    className="text-white cursor-pointer p-1 border-2 w-full rounded" 
                  />
                </div>
                <div>
                  <label className="block mb-1 font-semibold">CAPTION (Optional)</label>
                  <input 
                    type="text" 
                    name="caption" 
                    className="w-full p-2 bg-[#0a0a0a] rounded border border-gray-600" 
            ...
            {infoImages.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {infoImages.map((img) => (
                  <div key={img.id} className="bg-[#1a1a1a] rounded-lg overflow-hidden">
                    <Image
                      src={img.image_url}
                    ...
                    />
                    <div className="p-4">
                      <p className="text-sm text-gray-400 mb-2">{img.caption || 'No caption'}</p>
                      <button 
                        onClick={() => handleDeleteInfoImage(img)} 
                        className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-400 text-sm cursor-pointer w-full"
                      >
                        Delete
                      </button>
                    </div>
